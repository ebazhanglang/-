<template>
  <div class="job-info">
    ...
    <el-form label-width="220px">
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="loadEmployeesJob">保存更新</el-button>
...
import { getEmployeesJob, saveEmployeesJob } from '@/api/EmployeeSimple'
export default {
  name: 'JobInfo',
  data() {
    return {
  ...
  created() {
    this.loadingEmployeesJob()
    this.loadEmployeesJob()
  },
  methods: {
    async loadingEmployeesJob() {
      this.formData = await getEmployeesJob(this.userId)
    },
    async loadEmployeesJob() {
      await saveEmployeesJob(this.formData)
    }
  }
}
</script>
